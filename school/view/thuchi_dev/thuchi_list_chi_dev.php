<?php 
	...
	$array_header_thuchi =  array("stt"=>array("Stt",array("style"=>"text-align:center; width:3%")),
								...
								"doituong"=>array("Đối tượng",array("style"=>"text-align:center")),
								"ngay"=>array("Ngày",array("style"=>"text-align:center")),
								"hoten"=>array("Khách hàng",array("style"=>"text-align:center")),
	...
	if($array_list_thuchi != NULL)
	{
		foreach($array_list_thuchi as $thuchi)
		{
			...
			$array_row_thuchi["group_name"] 			= array($thuchi["group_name"],array("style"=>"text-align:center;"));
			...
			$link_xoa = "";
			$link_xoa 				= $this->Html->link(array("controller"=>"thuchi","action"=>"xoa","params"=>array($thuchi["id"],$thuchi["type"],$thuchi["type_account"],"add")));
	...
	{
		$array_row_thuchi["nodata"] 		= array("Không có dữ liệu",array("style"=>"text-align:center;","colspan"=>"8"));	
		$str_row_thuchi .= $this->Template->load_table_row($array_row_thuchi);	
	}
	if($this->get_config("thuchi_tong") == "yes")
	{
		...
		$array_row_thuchi["lo"] 	= array("Lỗ: $lo",array("style"=>"text-align:center;color:yellow;font-weight:bold;","colspan"=>"2","id"=>"lo"));
