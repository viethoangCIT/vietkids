...
<?php
	...
	if($array_thuchi)
	{
		...
		$amount  = number_format( $array_thuchi[0]["amount"]);
	...
	$bieuphi = "";
	$str_thongtin_tre = "";
	if($type == "thu" && $group_code == "hocphi")
	{
		$str_thongtin_tre = "<h4><b style='color:#2f83b7'>Họ và tên:</b> $customer_name</h4>";
		$str_thongtin_tre .= "<h4><b style='color:#2f83b7'>Lớp:</b> $classroom_name</h4>";
		$bieuphi = "<b style='color:#2f83b7'>Biểu phí:</b>";
		$phieuthu 	= $this->Html->link(array("controller"=>"revenue","action"=>"phieu_thu_hoc_phi","params"=>array($id)));
	}
	if($type == "thu" && $group_code == "khac") 
	{
		$phieuthu 	= $this->Html->link(array("controller"=>"revenue","action"=>"phieu_thu_khac","params"=>array($id)));	
	}
	if($type == "chi" && $group_code == "khac") 
	{
		$phieuthu 	= $this->Html->link(array("controller"=>"revenue","action"=>"phieu_chi","params"=>array($id)));
	}
	
	$phieuthu	= $this->Template->load_link("download","In phiếu $type",$phieuthu,array("target"=>"_blank"));
