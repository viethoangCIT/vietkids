...
<?php
	...
	$str_button_print = $this->Template->load_button(array("type"=>"button","onclick"=>"goto_print('$id_classroom')"),"In biểu phí");
	$str_form_fee .= $this->Template->load_form_row(array("title"=>"Tháng","input"=>$str_combo_thang.$str_button_print));
	...
	//lấy dữ liệu biểu phí của trẻ từ bảng fee_customer
		...
		foreach($array_fee_customer as $fee_customer)
		{
			...
			$student_id_service = "";
			$str_student_service = "<table border='0'>";
			...
			$str_student_service .= "<tr><td colspan='6'>Khác: $str_combo_student_service $str_hidden_fee $str_student_service_button_luu <br> $str_button_copy </td></tr>";
			...
			$array_row_fee["service"]  	= array("<div style='width:100%;overflow-y: scoll; border: 0px solid red'>".$str_student_service."</div>",array("style"=>"text-align:left; "));
...
function goto_print(id_classroom)
{
	month_year = document.getElementById("month_year").value;

	window.location.href = "/fee/print_fee/"+id_classroom+"/"+month_year;
}
</script>
