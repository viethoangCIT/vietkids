<?php
	...
	$array_header_xep_lop =  array("stt"=>array("Stt",array("style"=>"text-align:center; width:3%")),
							...
							"year"=>array("Năm",array("style"=>"text-align:center; width:12%")),
							"option"=>array("Tùy chọn",array("style"=>"text-align:center; width:12%")),
	...
	if($array_danhsach)
	{
		foreach($array_danhsach as $danhsach)
		{
			...
			$array_row_xep_lop["year"] 	= array($danhsach["year"],array("style"=>"text-align:center;"));


			$link_xoa=$this->Template->load_link("del","Xóa","/classroom/arrange_list?id=".$danhsach["id"]."&act=del");

			$array_row_xep_lop["option"] 	= array($link_xoa,array("style"=>"text-align:center;"));
