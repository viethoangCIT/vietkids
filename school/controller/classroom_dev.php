...
class classroom extends Main
{
	...
	function arrange_list()
	{
		...
		//kiểm tra có xóa học sinh khỏi lớp không
		if(isset($_GET["act"]))
		{
			$act = $_GET["act"];
			if($act == "del" )
			{
				...
				$id = $_GET["id"];
				$id_customer = $this->Arrange->get_value(array("fields"=>"id_fullname","conditions"=>"id='$id'"));				

				//cập nhật lại trạng thái học sinh chưa xếp lớp
				$this->loadModel("Customer");
				
				$array_customer = NULL;
				$array_customer['status'] = 0;
				$array_customer['id'] = $id_customer;
				$this->Customer->save($array_customer);
				
				//xóa khỏi bảng xếp lớp
				if($id!="") $this->Arrange->delete($id);
				
	
					
			}	
		}
		//kết thúc: xóa học sinh khỏi lớp
