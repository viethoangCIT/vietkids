...
class fee extends Main
{
	...
	function detail($id_fee = "")
	{
		...
		//BEGIN: KIỂM TRA CÓ DỮ LIỆU SUBMIT ĐỂ LƯU
		...
		}//END: if(isset($_POST["data"])==true)
		//END: KIỂM TRA CÓ DỮ LIỆU SUBMIT ĐỂ LƯU
		
		//su dung ham find de load du lieu tu bang fee_detail hien thi ra danh sach dich vu cua bieu phi
		$array_fee_info = $this->Fee->find("all",array("conditions"=>"id = '$id_fee'"));


		//su dung ham find de load du lieu tu bang fee_detail hien thi ra danh sach dich vu cua bieu phi
		$array_detail = $this->FeeDetail->find("all",array("conditions"=>"`id_fee` = '$id_fee'"));
			
		  //BEGIN: LƯU VÀO CỘT SERVICE VÀ TOTAL CỦA BẢNG FEE
		  ...
		  if($array_fee_detail != NULL)
		  {
			  $price = 0;
			  for($i = 0; $i < count($array_fee_detail); $i++)
			  {
				  if(isset($array_fee_detail[$i]["select"]))
				  {
					  $price = str_replace(",","",$array_fee_detail[$i]["price"]);
					  if($price > 0) $total_price += $price;
					  $id_service = $array_fee_detail[$i]["id_service"];
					  ...
					  $str_service .= $array_fee_detail[$i]["service_name"].$kitu.$price.$kitu.$id_service.$kitu.$code_service.$kitu_dong;
				  }
			  }
		  }
		  
		  $array_save_fee = NULL;
		  $array_save_fee["str_service"] = $str_service;
		  $array_save_fee["total_price"] = $total_price;
		  $array_save_fee["id"] = $id_fee;
		  $this->FeeDetail->save($array_save_fee);
		  $this->Fee->save($array_save_fee);
		  //END: LƯU VÀO CỘT SERVICE VÀ TOTAL CỦA BẢNG FEE
	...
	function add($tmp_id = "")
	{
		...
		$them_sp = $this->View->render('add_fee.php',array("array_sua"=>$array_sua,"array_service"=>$array_service));
		echo $them_sp;		
	}
	...
	function arrange($id_classroom = "")
	{
		...
		$danhsach = $this->View->render('arrange_fee.php',array("month"=>$month,"array_hocsinh"=>$array_hocsinh,"array_lop_hoc"=>$array_lop_hoc,"array_dichvu"=>$array_dichvu,"classroom_name"=>$classroom_name,"fee_name"=>$fee_name,"total_price"=>$total_price,"str_fee_detail"=>$str_fee_detail,"id_classroom"=>$id_classroom,"id_fee"=>$id_fee,"array_fee_customer"=>$array_fee_customer));
		echo $danhsach;	
	}

	function print_fee($id_classroom = "", $month = "")
	{
		
		


		//truy vấn dữ liệu hết tất cả học sinh trong lớp từ bảng arranges với điều kiện id_class = $id_class
		$this->loadModel("HocSinh","arranges");
		$array_hocsinh = $this->HocSinh->find("all",array("order"=>"fullname ASC","fields"=>"id_fullname,fullname,classroom_name","conditions"=>"(`id_classroom` = '$id_classroom')"));
		
		//truy vấn dữ liệu biểu phí của trẻ trong bảng fee_customer_details
		// theo điều kiện id_classroom = $id_classroom, month_year = $month,
		$this->loadModel("FeeCustomer","fee_customers");
		$month = strtotime("01-".$month);
		$str_month = date("Y-m-d",$month);

		//tạo mảng để chứa id học sinh đã có trong biểu phí học sinh
		$array_student = null;


		//duyệt tất cả biểu phí của trẻ có trong bảng fee_customers
		$array_fee_customer = $this->FeeCustomer->find("all",array("conditions"=>"(`id_classroom` = '$id_classroom') AND (`month_year` = '$str_month')"));
		$num_fee_student = 0;

		foreach($array_fee_customer as $fee)
		{
			//lưu lại id học sinh đã có trong bảng fee_customer
			$array_student[$num_fee_student] = $fee["id_customer"]; 
			$num_fee_student++;


			$phieuin = $this->View->render('print_fee.php',array("fee"=>$fee),false);
			echo $phieuin;	
		...
		//truy vấn dữ liệu fee_classroom để lấy biểu phí của lớp
		$this->loadModel("FeeClassroom","fee_classrooms");
		$array_fee_classroom = $this->FeeClassroom->find("all",array("conditions"=>"(`id_classroom` = '$id_classroom') AND (`month_year` = '$str_month')"));

		foreach($array_hocsinh as $hocsinh)
		{
			//nếu id học sinh hiện tại chưa có trong mảng array_student thì đưa vào view print_fee.php
			if(!in_array($hocsinh["id_fullname"],$array_student))
			{
				$hocsinh["customer_name"] = $hocsinh["fullname"];
				$hocsinh["total_amount"] = $array_fee_classroom[0]["total_price"];
				$hocsinh["month_year"] = $str_month;
				$hocsinh["str_service"] = $array_fee_classroom[0]["str_service"];
				$phieuin = $this->View->render('print_fee.php',array("fee"=>$hocsinh),false);
				echo $phieuin;
			}//END:if(!in_array($hocsinh["id_fullname"],$array_student))
			
		}//END: foreach($array_hocsinh as $hocsinh) 
		
	}//END: function print_fee($id_classroom = "", $month = "")
}
?>